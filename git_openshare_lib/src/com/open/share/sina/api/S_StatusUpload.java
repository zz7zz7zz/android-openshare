...
public class S_StatusUpload extends SinaNetRunnable {

	private Bundle mBundle;
	
	public S_StatusUpload(Bundle mBundle,IOpenResponse listener)
	{
	...
	public void loadData() {
		...
		this.reqUrl=SinaShareImpl.URL_STATUSES_UPLOAD;
		...
		this.formMultParamList.add(new BasicNameValuePair("status",URLEncoder.encode(mBundle.getString(OpenManager.BUNDLE_KEY_TEXT))));
...
//		this.formMultParamList.add(new BasicNameValuePair("long", mBundle.getString(OpenManager.BUNDLE_KEY_LONGITUDE)));
		...
		byte[] postBody=SFormBodyUtil.getData(reqHeards, formMultParamList, mBundle.getString(OpenManager.BUNDLE_KEY_IMGPATH));
...
LogUtil.v("StatusUploadMsg handleData():", "---"+str);
		if(!TextUtils.isEmpty(str))
		{
			...
			try {
				...
				if(null!=object)
				{
					...
					{
						if(object.has("original_pic"))
						{
							ret.backObj=object.optString("original_pic");
LogUtil.v("sina original_pic:", ""+ret.backObj.toString());
						}
					}
					else
					{
